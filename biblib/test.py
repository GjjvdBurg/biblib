  ...
  year         = 2013,
    ...
    def test_month_num(self):
        def test(string, expect):
            entry = Entry(
                [("month", string)], field_pos={"month": Pos.unknown}
            )
    ...
    def test_date_key(self):
        def test(year, month, expect):
            ...
            entry = Entry(
                fields, field_pos={"year": Pos.unknown, "month": Pos.unknown}
            )
            got = entry.date_key()
...
class NameParserTest(unittest.TestCase):
    ...
    def test_and(self):
        ...
        self.assertEqual(
            p("A B { and } C D"), [Name("A B { and } C", "", "D", "")]
        )
        self.assertEqual(
            p("A B {\\ and } C D"), [Name("A B", "{\\ and }", "C D", "")]
        )
    ...
    def __test_names(self, *tests):
        for test in tests:
            ...
            self.assertEqual(
                names, [Name(*test[1:])], "parsing {!r}".format(test[0])
            )
...
class NamePrettyTest(unittest.TestCase):
    ...
    def test_basic(self):
        self.__test(
            "{first}{von}{jr}{last}", lambda n: n.first + n.von + n.jr + n.last
        )
        self.__test(
            "{first} {von} {last} {jr}",
        ...
        self.__test(
            "{von} {last}, {first}, {jr}",
            lambda n: self.__clean(
                "{0.von} {0.last}, {0.first}, {0.jr}".format(n)
            ),
        )
...
class TeXToUnicodeTest(unittest.TestCase):
    ...
    def test_ligatures(self):
        self.assertEqual(
            tex_to_unicode(r"a--b---c-{-}d"), "a\u2013b\u2014c--d"
        )
